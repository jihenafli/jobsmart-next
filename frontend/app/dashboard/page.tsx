...
function AgentBadge({ name, active }: { name: string; active: boolean }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 10px', borderRadius:20,
      ...
      <div style={{ width:5, height:5, borderRadius:'50%', background: active?'var(--green)':'var(--muted)' }}/>
      {name}
    </div>
  );
}
function ScoreRing({ score }: { score: number }) {
  const color = score>=80?'var(--green)':score>=65?'var(--amber)':'var(--muted)';
  return (
    <div style={{ width:52, height:52, borderRadius:'50%', background:`${color}11`, color, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:14, border:`2px solid ${color}` }}>
      {score}%
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const searchJobs = async () => {
    ...
    await withAgent(['Hunter Agent'], async () => {
      try {
        const res = await api.post('/api/jobs/search', { country, platforms: [] });
        setJobs(res.data.jobs || []);
        if (!res.data.jobs?.length) setError('Aucune offre trouvée. Vérifie JSEARCH_API_KEY');
        setStep(2);
      } catch (e:any) { setError(e.response?.data?.error || 'Erreur recherche'); }
      finally { setJobsLoading(false); }
    });
  ...
  const generateLetter = async (job: any) => {
    setActiveJob(job); setLetterLoading(true); setLetter(''); setSendResult(null);
    setRecEmail(job.companyEmail || ''); setStep(3);
    await withAgent(['Apply Agent','Email Agent'], async () => {
      try { const res = await api.post('/api/applications/generate', { job }); setLetter(res.data.coverLetter); }
      catch (e:any) { setError(e.response?.data?.error || 'Erreur génération'); }
      finally { setLetterLoading(false); }
  ...
  const sendApp = async () => {
    setSending(true); setError('');
    await withAgent(['Email Agent'], async () => {
      try { const res = await api.post('/api/applications/send', { job:activeJob, coverLetter:letter, recipientEmail:recEmail }); setSendResult(res.data); }
      catch (e:any) { setError(e.response?.data?.error || 'Erreur envoi'); }
      finally { setSending(false); }
    });
  };
  const loadHistory = async () => {
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)' }}>
      <nav style={{ background:'var(--bg2)', borderBottom:'1px solid var(--border)', padding:'0 24px', height:56, display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:100 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <Link href="/interview" className="btn" style={{ fontSize:11, padding:'5px 10px' }}>🎤 Interview</Link>
          <span style={{ fontSize:12, color:'var(--muted)' }}>{user?.name}</span>
          <button className="btn" style={{ fontSize:11 }} onClick={logout}>Déco</button>
        </div>
      </nav>
      ...
      <div style={{ maxWidth:840, margin:'0 auto', padding:'28px 20px' }}>
        <div style={{ display:'flex', marginBottom:24, background:'var(--bg2)', borderRadius:12, border:'1px solid var(--border)', overflow:'hidden' }}>
          {STEPS.map((s,i)=>(
            <button key={i} onClick={()=>i<step&&setStep(i)}
              style={{ flex:1, padding:'11px 4px', border:'none', borderRight: i<STEPS.length-1?'1px solid var(--border)':'none',
                background: i===step?'var(--green)':'transparent',
                color: i===step?'#000':i<step?'var(--green)':'var(--muted)', cursor: i<step?'pointer':'default', fontSize:12, fontWeight:600 }}>
              <span style={{ display:'block', fontSize:9, opacity:.6 }}>0{i+1}</span>{s}
            </button>
          ))}
        </div>
        ...
        {step===0 && (
          <div className="glass" style={{ padding:28 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6 }}>
              <AgentBadge name="Resume Agent" active={cvLoading}/><h2 style={{ fontSize:20 }}>Télécharge ton CV</h2>
            </div>
            {!cv &&!cvLoading && (
              <>
                <div onClick={()=>fileRef.current?.click()} style={{ border:'2px dashed var(--border)', borderRadius:12, padding:'52px 24px', textAlign:'center', cursor:'pointer' }}>
                  <div style={{ fontSize:42, marginBottom:12 }}>📄</div>
                  <p style={{ fontWeight:600 }}>Clique pour uploader ton CV</p><p style={{ fontSize:13, color:'var(--muted)' }}>PDF — max 20 MB</p>
                </div>
                <input ref={fileRef} type="file" accept=".pdf" style={{ display:'none' }} onChange={e=>e.target.files?.[0]&&uploadCV(e.target.files[0])}/>
              </>
            )}
            {cvLoading && <div style={{ textAlign:'center', padding:48 }}><p style={{ color:'var(--green)' }}>Resume Agent analyse ton CV...</p></div>}
            {cv &&!cvLoading && (
              <div>
                <div style={{ background:'var(--green-gl)', border:'1px solid rgba(0,214,143,.2)', borderRadius:10, padding:'12px 16px', display:'flex', gap:12, marginBottom:20 }}>
                  <div style={{ width:42, height:42, borderRadius:8, background:'var(--green)', display:'flex', alignItems:'center', justifyContent:'center', color:'#000', fontWeight:700 }}>CV</div>
                  <div style={{ flex:1 }}><p style={{ fontWeight:600, color:'var(--green)', fontSize:14 }}>CV analysé ✓</p>
                    <p style={{ fontSize:12, color:'var(--muted)' }}>{cv.analysis?.experience} · {cv.analysis?.education} · {cv.analysis?.skills?.length} compétences</p>
                  </div>
                  <button className="btn" onClick={()=>{setCv(null); if(fileRef.current)fileRef.current.value='';}}>Changer</button>
                </div>

                {/* BLOC PROFIL DÉTECTÉ COMPLET */}
                <div style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:10, padding:'16px 18px', marginBottom:20 }}>
                  <p style={{ fontSize:10, color:'var(--muted)', fontWeight:600, textTransform:'uppercase', letterSpacing:.6, marginBottom:14 }}>Profil détecté</p>
                  {cv.analysis?.jobTitles?.length>0 && (
                    <div style={{ marginBottom:12 }}><p style={{ fontSize:12, color:'var(--muted)', marginBottom:7 }}>Postes recommandés :</p>
                      <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>{cv.analysis.jobTitles.map((t:string)=><span key={t} style={{ fontSize:12, padding:'4px 11px', borderRadius:20, background:'rgba(139,92,246,.08)', color:'var(--purple)', border:'1px solid rgba(139,92,246,.2)' }}>{t}</span>)}</div>
                    </div>
                  )}
                  {cv.analysis?.skills?.length>0 && (
                    <div style={{ marginBottom:12 }}><p style={{ fontSize:12, color:'var(--muted)', marginBottom:7 }}>Compétences ({cv.analysis.skills.length}) :</p>
                      <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>{cv.analysis.skills.slice(0,15).map((s:string)=><span key={s} style={{ fontSize:12, padding:'3px 10px', borderRadius:20, background:'var(--green-gl)', color:'var(--green)', border:'1px solid rgba(0,214,143,.2)' }}>{s}</span>)}</div>
                    </div>
                  )}
                  {cv.analysis?.languages?.length>0 && (
                    <div style={{ marginBottom:12 }}><p style={{ fontSize:12, color:'var(--muted)', marginBottom:7 }}>Langues :</p>
                      <div style={{ display:'flex', gap:5 }}>{cv.analysis.languages.map((l:string)=><span key={l} style={{ fontSize:12, padding:'3px 10px', borderRadius:20, background:'rgba(245,158,11,.08)', color:'var(--amber)', border:'1px solid rgba(245,158,11,.2)' }}>{l}</span>)}</div>
                    </div>
                  )}
                  {cv.analysis?.strengths?.length>0 && (
                    <div style={{ marginBottom:12 }}><p style={{ fontSize:12, color:'var(--muted)', marginBottom:7 }}>Points forts :</p>
                      <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>{cv.analysis.strengths.map((s:string)=><span key={s} style={{ fontSize:12, padding:'3px 10px', borderRadius:20, background:'rgba(79,142,247,.08)', color:'var(--blue)', border:'1px solid rgba(79,142,247,.2)' }}>{s}</span>)}</div>
                    </div>
                  )}
                  {cv.analysis?.atsScore && <div style={{ display:'flex', gap:10, marginTop:8 }}><p style={{ fontSize:12, color:'var(--muted)' }}>Score ATS :</p><span style={{ fontWeight:700, color: cv.analysis.atsScore>=70?'var(--green)':'var(--amber)' }}>{cv.analysis.atsScore}/100</span></div>}
                </div>

                <div style={{ display:'flex', justifyContent:'flex-end' }}><button className="btn btn-primary" onClick={()=>setStep(1)}>Continuer → Choisir le pays</button></div>
              </div>
            )}
          </div>
        )}
        {/* Les autres steps 1,2,3,4 restent identiques à ton code */}
        {step===1 && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div className="glass" style={{ padding:24 }}>
              <h2 style={{ fontSize:18, marginBottom:6 }}>Dans quel pays cherches-tu?</h2>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
                {COUNTRIES.map(c=><div key={c.code} onClick={()=>setCountry(c.code)} style={{ border: country===c.code?'2px solid var(--green)':'1px solid var(--border)', borderRadius:12, padding:'18px 12px', textAlign:'center', cursor:'pointer', background: country===c.code?'var(--green-gl)':'var(--bg3)' }}><p style={{ fontSize:28 }}>{c.label.split(' ')[0]}</p><p style={{ fontSize:13 }}>{c.label.split(' ').slice(1).join(' ')}</p></div>)}
              </div>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between' }}><button className="btn" onClick={()=>setStep(0)}>← Retour</button><button className="btn btn-primary" onClick={searchJobs}>{jobsLoading?'🤖 Hunter Agent...':'🤖 Lancer la recherche →'}</button></div>
          </div>
        )}
        {step===2 && <div>{/* ton code offres - garde le même */}</div>}
        {step===3 && <div>{/* ton code candidature - garde le même */}</div>}
        {step===4 && <div>{/* historique */}</div>}
      </div>
    </div>
  );
}