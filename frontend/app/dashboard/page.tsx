      ...
      <div style={{ maxWidth:840, margin:'0 auto', padding:'28px 20px' }}>
            ...
            {!cv && !cvLoading && (
              <>
                <div onClick={()=>fileRef.current?.click()}
                  ...
                  <p style={{ fontSize:13, color:'var(--muted)', marginTop:4 }}>PDF — max 20 MB</p>
                </div>
              ...
              <div>
                ...
                <div style={{ background:'var(--green-gl)', border:'1px solid rgba(0,214,143,.2)', borderRadius:10, padding:'12px 16px', display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <p style={{ fontSize:12, color:'var(--muted)', marginTop:2 }}>
                      {cv.analysis?.experience} · {cv.analysis?.education} · {cv.analysis?.skills?.length} compétences
                    </p>
                  </div>
                ...
                <div style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:10, padding:'16px 18px', marginBottom:20 }}>
                  ...
                  {cv.analysis?.strengths?.length>0 && (
                    <div style={{ marginBottom:12 }}>
                      <p style={{ fontSize:12, color:'var(--muted)', marginBottom:7 }}>Points forts :</p>
                      <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
                        {cv.analysis.strengths.map((s:string)=>(
                          <span key={s} style={{ fontSize:12, padding:'3px 10px', borderRadius:20, background:'rgba(79,142,247,.08)', color:'var(--blue)', border:'1px solid rgba(79,142,247,.2)' }}>{s}</span>
                        ))}
                      </div>
                    </div>
                  )}

                  {cv.analysis?.atsScore && (
                    <div style={{ display:'flex', alignItems:'center', gap:10, marginTop:8 }}>
                      <p style={{ fontSize:12, color:'var(--muted)' }}>Score ATS :</p>
                      <span style={{ fontSize:13, fontWeight:700, color: cv.analysis.atsScore>=70?'var(--green)':'var(--amber)' }}>
                        {cv.analysis.atsScore}/100
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {history.map((app,i)=>(
                  <div key={i} className="glass" style={{ padding:'14px 20px', display:'flex', alignItems:'center', gap:14 }}>
                    ...
                    <span style={{ fontSize:11, padding:'3px 10px', borderRadius:20, background:'var(--green-gl)', color:'var(--green)', border:'1px solid rgba(0,214,143,.2)' }}>
        ...
        {step<4 && (
          <div style={{ textAlign:'center', marginTop:20 }}>
            <button style={{ background:'none', border:'none', color:'var(--muted)', fontSize:12, cursor:'pointer' }} onClick={loadHistory}>
