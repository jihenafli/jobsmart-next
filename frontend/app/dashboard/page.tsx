...
/**
 * Transforme la réponse texte du modèle
 * en objet JavaScript.
 *
 * Le modèle NE génère PAS de JSON.
 */
function parseAIResponse(text) {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      lower.startsWith("competences:") ||
      lower.startsWith("compétences:")
    ) {
      result.skills = extractList(line);
    }

    else if (
      lower.startsWith("job titles:") ||
      lower.startsWith("postes:") ||
      lower.startsWith("postes recommandes:") ||
      lower.startsWith("postes recommandés:")
    ) {
      result.jobTitles = extractList(line);
    }

    else if (
      lower.startsWith("languages:") ||
      lower.startsWith("langues:")
    ) {
      result.languages = extractList(line);
    }

    else if (
      lower.startsWith("experience:") ||
      lower.startsWith("expérience:")
    ) {
      result.experience = line
        .replace(/^experience\s*:/i, "")
        .replace(/^expérience\s*:/i, "")
        .trim();
    }
  ...
  try {
    ...
    if (!req.file) {
      return res.status(400).json({
        error: "Aucun CV PDF envoyé."
      });
    }

    filePath = req.file.path;

    console.log("📄 CV reçu :", req.file.originalname);
    ...
    console.log("📝 Texte CV extrait :", cvText.length, "caractères");


    // ==========================================
    // 2. ANALYSE AVEC GPT-OSS-20B
    // ==========================================

    const prompt = `
Tu es un recruteur expert et un système ATS.

Analyse le CV ci-dessous.

Ton objectif est d'identifier précisément :

1. Les compétences techniques
2. Les compétences professionnelles
3. Les postes correspondant au profil
4. Les langues
5. Le niveau d'expérience
6. La formation principale

IMPORTANT :

Tu ne dois PAS répondre en JSON.

Tu dois répondre exactement avec ces lignes :

Skills: compétence1, compétence2, compétence3
Job Titles: poste1, poste2, poste3
...
- Utiliser uniquement les informations présentes dans le CV.
- Les compétences doivent être courtes et précises.
- Évite les doublons.
- Maximum 15 compétences.
- Maximum 5 postes.
- Maximum 5 langues.
- Répondre uniquement avec les 5 lignes demandées.
- Pas de Markdown.
- Pas de JSON.
...
`;
      ...
      messages: [
        {
          role: "system",
          content:
            "Tu es un expert en analyse de CV et recrutement."
        },
        {
          role: "user",
          content: prompt
    ...
    // ==========================================
    // 5. SUPPRESSION DU PDF TEMPORAIRE
    // ==========================================
